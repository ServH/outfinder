...
- Previous story intelligence — [_bmad-output/implementation-artifacts/archive/epic-13/13-2-background-removal-native-module.md](./13-2-background-removal-native-module.md) (module contract, error bridge, on-device smoke baseline)
...
_Generated by `bmad-code-review` — 3-layer adversarial review (Blind Hunter + Edge Case Hunter + Acceptance Auditor). 2026-04-19._

**AC Audit: 10/10 PASS** — All acceptance criteria fully implemented.

#### Patch (fix before merge)

- [x] [Review][Patch] `StyleSheet` imported in `ArmarioCaptureScreen` — `StyleSheet.absoluteFill` violates CLAUDE.md "never StyleSheet" rule; replace with `style={{ position: "absolute", top: 0, left: 0, bottom: 0, right: 0 }}` or use `className="absolute inset-0"` if NativeWind supports it for `CameraView`. [`src/screens/armario/ArmarioCaptureScreen.tsx`]
- [ ] [Review][Patch] `useWardrobeStore.getState()` called inside JSX render — stale read, not reactive; replace with hook selector `useWardrobeStore(s => s.items.length)` for the badge display in Settings dev row. [`src/screens/Settings.tsx:302`]
- [ ] [Review][Patch] Back chevron on `ArmarioCaptureScreen` missing `accessibilityState={{ disabled: disableControls }}` — VoiceOver can activate a disabled control; library and capture buttons have the same gap. [`src/screens/armario/ArmarioCaptureScreen.tsx`]
- [ ] [Review][Patch] `RootStackParamList.ArmarioRoot.params` typed as raw union — should use `NavigatorScreenParams<ArmarioStackParamList>` to properly constrain `screen`/`params` pairs and prevent misuse in Stories 13.4a/b. [`src/navigation/types.ts`]
- [ ] [Review][Patch] `handlePaywallDismiss` calls `setPaywallVisible(false)` without `isMounted.current` guard — if modal animation completes after unmount, sets state on unmounted component; add `if (!isMounted.current) return;` guard. [`src/screens/armario/ArmarioPreviewScreen.tsx`]
- [ ] [Review][Patch] `animation: "fade"` in `ArmarioStack.tsx` not guarded by Reduce Motion — CLAUDE.md mandates checking `useReducedMotion()` before any animation; conditionally set `animation` or omit to use system default. [`src/navigation/ArmarioStack.tsx:11`]
- [ ] [Review][Patch] `WardrobePersistenceError` missing `Object.setPrototypeOf(this, WardrobePersistenceError.prototype)` in constructor — `instanceof` checks can silently fail after Babel/Hermes transpilation of custom Error subclasses; add the well-known fix. [`src/lib/armario/wardrobeErrors.ts`]
- [ ] [Review][Patch] Test #8 (unmount-safety) console.error spy set up after the action — spy must be installed before the `act(async () => { fireEvent.press(...) })` call to capture any early state-update warnings; move spy setup to before `fireEvent`. [`src/screens/armario/ArmarioCaptureScreen.test.tsx`]
- [ ] [Review][Patch] ArmarioPreviewScreen test #1 renders component twice without `cleanup()` between renders — second render accumulates on top of first; `getByTestId` may match the stale first instance's button; add `cleanup()` between the two `render()` calls. [`src/screens/armario/ArmarioPreviewScreen.test.tsx`]
- [ ] [Review][Patch] Test #4 library cancel sub-case uses `.mockResolvedValue` (permanent) instead of `.mockResolvedValueOnce` — cancel and grant sub-cases share the same mock resolution; use `Once` per sub-case for isolation. [`src/screens/armario/ArmarioCaptureScreen.test.tsx`]
- [ ] [Review][Patch] `pickFromLibrary` missing `isCapturing.current` guard — shutter tap sets `isCapturing.current = true` but library button only checks `processing` state (which lags a render cycle); add `if (isCapturing.current) return;` at the top of `pickFromLibrary`. [`src/screens/armario/ArmarioCaptureScreen.tsx`]
- [ ] [Review][Patch] `isBackgroundRemovalError` type guard accepts any string-kinded object — a future `WardrobePersistenceError` (kind: "paywall") could match and silently map to `visionFailed` copy; narrow the guard to explicitly check `kind` is in `["noSubject", "visionFailed", "ioFailed"]`. [`src/screens/armario/ArmarioCaptureScreen.tsx:36-40`]
- [ ] [Review][Patch] `Settings.test.tsx` `@react-navigation/native` mock only stubs `useNavigation` — any future hook (`useRoute`, `useFocusEffect`, etc.) used by Settings or its child components will receive `undefined` and throw; add minimal stubs for `useRoute` and `useFocusEffect`. [`src/screens/Settings.test.tsx`]

#### Defer (pre-existing or out-of-scope)

- [x] [Review][Defer] `File.delete()` SDK 55 class-based API sync/async unclear — if `delete()` returns a `Promise<void>`, the `try/catch` in `deleteCutoutTmp` silently swallows rejections; verify SDK 55 types before 13.3b lands. [`src/screens/armario/ArmarioPreviewScreen.tsx`] — deferred, needs SDK 55 type verification
- [x] [Review][Defer] `handlePaywallDismiss` deletes cutout unconditionally — if user successfully upgrades mid-paywall and 13.3b's real save runs, `cutoutUri` will already be deleted; documented as acceptable in Completion Notes; 13.3b owns the real fix. — deferred, 13.3b scope
- [x] [Review][Defer] `handleRetake` double-goBack risk — native swipe-back gesture + Retake tap in same frame could issue two `navigation.goBack()` calls; React Navigation handles duplicate pops gracefully on a stack with screens below. — deferred, pre-existing RN pattern
- [x] [Review][Defer] Stub happy-path leaves tmp file orphaned — on successful `saveCutoutAsWardrobeItem` the tmp PNG is never deleted; explicitly documented in Completion Notes as acceptable for 13.3a; 13.3b adds cleanup in the real save path. — deferred, 13.3b scope
- [x] [Review][Defer] `AccessibilityInfo.announceForAccessibility` fires on `t` identity change — `useEffect` dep on `[t]` means announcement re-fires if i18next re-creates `t`; theoretical since `t` is stable in i18next sessions. — deferred, theoretical edge case
- [x] [Review][Defer] Test #1 permission re-render isolation gap — test validates ref guard on same-state re-render but not on undetermined → granted state transition; test currently passes; full variant coverage deferred. — deferred, test quality
- [x] [Review][Defer] Missing `testID` on `ActivityIndicator` inside processing overlay — tests can only assert the parent overlay container, not the spinner itself; minor gap. — deferred, test quality
- [x] [Review][Defer] `NSPhotoLibraryUsageDescription` duplicated in `ios.infoPlist` AND `expo-image-picker` plugin `photosPermission` — Expo plugin writes the key; explicit `infoPlist` entry was added per spec as a backup; risk is both strings diverging in future; clean up redundant entry after confirming plugin writes it post-prebuild. [`app.json`] — deferred, intentional per spec; verify post-prebuild
