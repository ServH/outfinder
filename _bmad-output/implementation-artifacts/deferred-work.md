...
## Deferred from: code review of 14-13-epic14-ondevice-qa-happy-paths (2026-04-23)

- **D-14.13-1** — `src/components/PremiumPaywall.tsx:256–259` — `savedCombinationIds` + `currentCount` two-source-of-truth: palette visibility is gated by `currentCount > 0` but rendered palettes come from `savedCombinationIds`. All current callsites are consistent, but a future callsite could pass inconsistent values. Consider adding a runtime invariant assert in DEV or consolidating back to a single array as the source of truth.
- **D-14.13-2** — `src/contexts/PremiumContext.tsx:171–181` — `__dev_resetPremium` lives in the production context value and `useMemo` deps, adding a dead slot to the context shape in release builds. Not a runtime bug (function is a no-op outside `__DEV__`). Can be removed from the context interface and value with a `if (__DEV__)` conditional wrapping the whole definition + wiring in a future cleanup.
- **D-14.13-3** — `src/screens/armario/ArmarioPreviewScreen.tsx` — Dual paywall-visibility state: `gate.paywallVisible` (favorites-gate, not used for visibility here) and local `paywallVisible` (wardrobe error-gate). Intentional design but undocumented; add a comment clarifying the wardrobe paywall is triggered by `WardrobePersistenceError`, not by the standard favorites gate, to prevent future maintainer confusion.
- **D-14.13-4** — `src/components/armario/CompletenessBadge.tsx:67–74` — `transparent=true` correctly zeros `backgroundColor`, `paddingHorizontal`, and `paddingVertical`, but `borderRadius: 10` and `minHeight: 24` remain applied as dead styles. No visual artifact (nothing to clip, text drives actual height). Future polish: add `borderRadius: transparent ? 0 : 10, minHeight: transparent ? undefined : 24` for semantic completeness.
- **D-14.13-5** — `src/contexts/PremiumContext.tsx:151–163` — `__dev_resetPremium` doesn't call `Purchases.logOut()` or equivalent RevenueCat reset. After a Metro reload, `init()` re-runs and RevenueCat re-hydrates `isPremium=true` from sandbox entitlement, negating the reset. Add a comment warning dev users of this limitation; optionally add a `Purchases.logOut()` call gated by `__DEV__`.
