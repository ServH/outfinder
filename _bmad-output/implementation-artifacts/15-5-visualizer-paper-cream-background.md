...
- [x] **Task 5 — CI gates + on-device visual smoke + AC verification** (AC: #4, #5)
  ...
  - [ ] On-device visual smoke per AC #5 — Alejandro on iPhone 16 Pro, both ES + EN locales, at least 3 Wada combos covering luminance range. ALSO smoke the tab bar transition: Mis Looks tab → Colors tab → tap a color → tap a combination → enter Visualizer → confirm tab bar bg is paper-cream throughout (no flash to warm sand) → back to Combinations → confirm no flicker. Record in Completion Notes List + Change Log: date, device, locales, combos tested, final Aureola alpha pair shipped (`0x40`/`0x1a` default OR adjusted pair if Alejandro requests tune), tab bar transition smoke result. **— PENDING ALEJANDRO**
  - [x] AC verification (point-by-point): AC #1 ✓ (1 token swap + 1 import delete + 1 JSX delete in OutfitVisualizer.tsx). AC #2 ✓ (1 colors-array tune in Aureola.tsx, no other props/comments changed). AC #3 ✓ (WarmBackground.tsx deleted; presentation.test.tsx loses 1 import + 1 describe; OutfitVisualizer.test.tsx loses 1 it-block + 1 inner expect; CustomTabBar.tsx warm-screen crossfade collapsed; `git grep WarmBackground src/` returns 0; `wadaTokens.warmBg` token + Tailwind alias + theme.test.ts assertion preserved). AC #4 ✓ (tsc/lint/test deltas exact; no new skips; targeted regressions clean). AC #5 ⏸ on-device smoke pending Alejandro.
...
- ⏸ **AC #5 on-device smoke handoff to Alejandro**: boot iPhone 16 Pro, navigate `ColorHome → tap color → tap combination → OutfitVisualizer`. Expected: parent canvas reads as paper-cream `#fafaf8` (matches `ColorHome`/`FavoritesList`/`BrowseAllColors`), Aureola halo visible behind silhouettes without overpowering Wada garments. Test ≥3 luminance-spread combos in ES+EN locales. Tab bar bg should stay paper-cream across Combinations ↔ OutfitVisualizer transition (no warm flash). If Aureola reads "too weak" → bump to `0x4d`/`0x20`; "too strong" → ease to `0x33`/`0x14`; "edged" → `0x40`/`0x10`. Per spec, the dev agent CANNOT verify FR11 via tests alone (perceptual tune); Alejandro records date/device/locales/combos/final-alpha-pair in this file's Change Log post-smoke. Branch ready for `bmad-code-review` adversarial pass + on-device smoke gate before merge to `epic-15`.
...
- 2026-04-27 — Story 15.5 implemented on branch `story/15-5-visualizer-paper-cream-background` off `epic-15` HEAD `baa31aa` by `bmad-dev-story` (Claude Opus 4.7). 5 tasks, 6 files touched (1 deleted). CI: tsc 0 unchanged, lint 2 pre-existing unchanged, pnpm test 961/3/964 (delta -3 net, exact). Aureola alpha tuned to calibrated default `0x40`/`0x1a`. CustomTabBar warm-screen crossfade fully collapsed. Status moved `ready-for-dev → review`. AC #1/#2/#3/#4 ✓; AC #5 (on-device visual smoke by Alejandro) PENDING — final Aureola alpha pair shipped will be recorded here post-smoke.
